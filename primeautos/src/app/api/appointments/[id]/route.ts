...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!Object.keys(updates).length) {
      return NextResponse.json({ success: false, message: "No hay campos válidos." }, { status: 400 });
    }
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Cita eliminada correctamente." }, { status: 200 });
  } catch (error) {
